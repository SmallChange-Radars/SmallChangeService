...
public class ClientPreferencesDaoOracleImpl implements ClientPreferencesDao {
	...
	public ClientPreferences getClientPreferences(String id) {
		...
		try (Connection connection = dataSource.getConnection();
				PreparedStatement stmt = connection.prepareStatement(queryForClintPreferences)) {
			stmt.setString(2, id);
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				String clientId = rs.getString("clientId");
				String investmentPurpose = rs.getString("investmentPurpose");
				String riskTolerance = rs.getString("riskTolerance");
				String incomeCategory = rs.getString("incomeCategory");
				String lengthOfInvestment = rs.getString("incomeCategory");

				ClientPreferences cf = new ClientPreferences(clientId, investmentPurpose, riskTolerance,
						incomeCategory, lengthOfInvestment);
				cfs.add(cf);
			}
		} catch (SQLException e) {
			throw new DatabaseException("Cannot execute queryForAllCars", e);
		}
	...
	public void insertClientPreferences(ClientPreferences clientPreferences) {
		try (Connection connection = dataSource.getConnection();
			PreparedStatement stmt = connection.prepareStatement(insertClientPreferences)) {
			stmt.setString(1, clientPreferences.getClientId());
			stmt.setString(2, clientPreferences.getInvestmentPurpose());
			stmt.setString(3, clientPreferences.getRiskTolerance().getName());
			stmt.setString(4, clientPreferences.getIncomeCategory().getName());
			stmt.setString(5, clientPreferences.getInvestmentLength().getName());
			stmt.executeUpdate();
		} catch (SQLException e) {
			throw new DatabaseException("Unable to insert ClientPreferences with id=" + clientPreferences.getClientId(), e);
		}
	}
	...
	public void updateClientPreferences(ClientPreferences clientPreferences) {
		try (Connection connection = dataSource.getConnection();
				PreparedStatement stmt = connection.prepareStatement(updateClientPreferences)) {
				...
				stmt.setString(5, clientPreferences.getClientId());
				stmt.executeUpdate();
			} catch (SQLException e) {
				throw new DatabaseException("Unable to Update ClientPreferences with id=" + clientPreferences.getClientId(), e);
			}
	}
}
